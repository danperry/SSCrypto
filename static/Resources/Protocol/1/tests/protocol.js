// Each test gets the Protocol version being tested: this one or a newer one.
...
export default {
	"get answers with a query result": async Protocol => {
	...
	"unknown messages get an error": async Protocol => {
		const reply = await Protocol.handle(await makeTree(), { type: "dance" });
		assert.equal(reply.type, "error");
	},

	"put accepts a newer branch, once": async Protocol => {
	...
	"put turns down a different owner": async Protocol => {
		const ours = await makeTree();
	...
	"put turns down a bad signature": async Protocol => {
		const ours = await makeTree();
	...
	"put keeps our newer copy of a delegated branch": async Protocol => {
		const ours = await makeTree();
		const theirs = structuredClone(ours);
